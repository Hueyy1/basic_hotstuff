...
func index(id types.ID) (byteIdx, bitIdx int) {
	i := int(id) - 1
	byteIdx = i / 8
	bitIdx = i % 8
	return
...
func id(byteIdx, bitIdx int) types.ID {
	return types.ID(1 + (byteIdx * 8) + bitIdx)
}
...
func (bf *Bitfield) Add(id types.ID) {
...
func (bf Bitfield) Contains(id types.ID) bool {
	byteIdx, bitIdx := index(id)
	if len(bf.data) <= byteIdx {
		return false
	}
...
func (bf Bitfield) ForEach(f func(types.ID)) {
	bf.RangeWhile(func(i types.ID) bool {
		f(i)
		return true
	})
}
...
func (bf Bitfield) RangeWhile(f func(types.ID) bool) {
