...
type ClientNodeClient interface {
	SendResponse(ctx context.Context, in *Response, opts ...gorums.CallOption)
}
...
type ClientServer interface {
	SendResponse(ctx gorums.ServerCtx, request *Response)
}

func RegisterClientServer(srv *gorums.Server, impl ClientServer) {
	srv.RegisterHandler("clientpb.Client.SendResponse", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*Response)
		defer ctx.Release()
		impl.SendResponse(ctx, req)
	})
}
...
// SendResponse is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (n *Node) SendResponse(ctx context.Context, in *Response, opts ...gorums.CallOption) {
	cd := gorums.CallData{
		Message: in,
		Method:  "clientpb.Client.SendResponse",
	}
