...
type Tree struct {
	...
	height       int
	branchFactor int
	treePosToID  []types.ID
	waitTime     time.Duration
}

// CreateTree creates the tree configuration.
func CreateTree(myID types.ID, bf int, ids []types.ID) Tree {
	if bf < 2 {
...
func (t Tree) Parent() (types.ID, bool) {
...
func (t Tree) SubTree() []types.ID {
	children := t.ChildrenOf(t.id)
	if len(children) == 0 {
		// safe since nil slices are equal to empty slices when iterating.
		return nil
	}
	subTreeReplicas := make([]types.ID, len(children))
...
func (t Tree) heightOf(replicaID types.ID) int {
